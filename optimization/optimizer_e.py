...
class OptimizerE(Optimizer):
    def solve(self, in_data: InData) -> OutData:
        ...
        for i, l in enumerate(in_data.Libraries):
            ...
            books_indexes_sorted_by_scores = in_data.Scores[l.Books].argsort()[::-1]
            books_sorted_by_scores = l.Books[books_indexes_sorted_by_scores]

            total_scores[i] = np.sum(books_sorted_by_scores[:(in_data.D * l.M)])
        ...
        lib_rate = total_scores / sign_up_times
