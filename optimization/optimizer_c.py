...
class OptimizerC(Optimizer):
    def solve(self, in_data: InData) -> OutData:
        ...
        for i, l in enumerate(in_data.Libraries):
            ...
            total_scores[i] = np.sum(in_data.Scores[l.Books])# / time_to_scan[i]
            # total_scores[i] = np.sum(in_data.Scores[l.Books]) / time_to_scan[i]  # TODO: only for E
